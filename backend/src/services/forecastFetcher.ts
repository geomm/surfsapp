...
export async function fetchForecastForBeach(beach: IBeach): Promise<IHourlyForecast[]> {
  ...
  const [marineResponse, windResponse] = await Promise.all([fetch(marineUrl), fetch(windUrl)]);
